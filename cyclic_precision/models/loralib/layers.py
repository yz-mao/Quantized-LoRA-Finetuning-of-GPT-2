...
class MultiLinear(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, lora_idx=0):
        def T(w):
            return w.transpose(0, 1) if self.fan_in_fan_out else w
        if self.r > 0 and not self.merged:
            result = F.linear(x, T(self.weight), bias=self.bias)
            result += (self.lora_dropout(x) @ self.lora_As[lora_idx].transpose(0, 1) @ self.lora_Bs[lora_idx].transpose(0, 1)) * self.scaling
            return result
        else:
            return F.linear(x, T(self.weight), bias=self.bias)


class MultiMergedLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
        self,
        in_features: int,
    ...
    def set_train_lora(self, lora_idx):
        for i in range(self.num_loras):
            requires_grad = (i == lora_idx)
            self.lora_As[i].requires_grad = requires_grad
            self.lora_Bs[i].requires_grad = requires_grad
    ...
    def forward(self, x: torch.Tensor, lora_idx):
        def T(w):
            return w.transpose(0, 1) if self.fan_in_fan_out else w
        if self.merged:
            return F.linear(x, T(self.weight), bias=self.bias)
            ...
            result = F.linear(x, T(self.weight), bias=self.bias)
            if self.r > 0:
                result += self.lora_dropout(x) @ T(self.merge_AB(lora_idx).T) * self.scaling
            return result
