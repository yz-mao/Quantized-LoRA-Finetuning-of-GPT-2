...
class MultiQuantizedLinear(nn.Linear, LoRALayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_As'):
            ...
            # Copy parameters from idx=0 to other idx>0 layers
            for i in range(1, self.num_loras):
                self.lora_As[i].data.copy_(copy.deepcopy(self.lora_As[0].data))
                self.lora_Bs[i].data.copy_(copy.deepcopy(self.lora_Bs[0].data))
