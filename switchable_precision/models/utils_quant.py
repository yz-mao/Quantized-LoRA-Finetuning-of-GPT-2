...
class Custom_QuestionAnsweringTrainer(QuestionAnsweringTrainer):

    def __init__(self, *args, lora_idx=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.lora_idx = lora_idx

    def train(self, **kwargs):
        # 在训练前激活指定的 LoRA 模块
        self.model.set_active_lora(self.lora_idx)
        return super().train(**kwargs)

    def evaluate(self, eval_dataset=None, lora_idx=None, **kwargs):
        # 在评估前设置 LoRA 模块
        if lora_idx is not None:
            self.model.set_active_lora(lora_idx)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # 如果训练到第二个或以上的 LoRA 模块，计算附加的 MSE loss
        if self.lora_idx[0] > 0:
            # 假设 input 的输出是 logits
            # 获得当前激活的 LoRA 的输出
            ...
            previous_lora_idx_list = [i - 1 for i in self.lora_idx]
            model.set_active_lora(previous_lora_idx_list)
            previous_output = model(**inputs)

            # 计算 MSE loss
            mse_loss = torch.nn.MSELoss()(current_output.start_logits, previous_output.start_logits)
            # print("start logits mse: ",mse_loss)

            # 将 MSE loss 添加到总 loss 中
            loss += 0.5 * mse_loss

            mse_loss = torch.nn.MSELoss()(current_output.end_logits, previous_output.end_logits)
            # print("end logits mse: ", mse_loss)

            # 将 MSE loss 添加到总 loss 中
            loss += 0.5 * mse_loss

            model.set_active_lora(self.lora_idx)

        return (loss, current_output) if return_outputs else loss
